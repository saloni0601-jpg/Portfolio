...
const BookDetails = () => {
    ...
    if (!book) return <div className="p-10 text-center font-serif text-[var(--color-header)]">Book not found</div>;
    ...
    return (
        ...
        >
            <div className="max-w-4xl mx-auto px-10 md:px-16 pt-8 pb-20">
                ...
                {/* Header */}
                <div className="mb-10 pb-8 border-b border-[var(--color-date)]/20">
                ...
                {/* Projects */}
                <div>
                    <h2 className="text-[13px] uppercase tracking-[0.2em] font-bold text-[var(--color-date)] mb-6">Projects and Chapters</h2>
                    <div className="space-y-4">
                        {book.projects.map((project, index) => (
                            ...
                            >
                                <Link to={`/book/${bookId}/chapter/${project.id}`} className="block group">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex flex-wrap gap-1.5">
                                                    ...
                                                    {project.technologies.length > 4 && (
                                                        <span className="text-[11px] text-[var(--color-date)] font-serif italic">+{project.technologies.length - 4} more</span>
                                                    )}
                                                </div>
                                            </div>
                                            <span className="font-serif text-[13px] text-[var(--color-subheading)] italic opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">Read</span>
                                        </div>
                                    </div>
                                </Link>
                            </motion.div>
